...
impl ApiPeripheral for Peripheral {
    ...
    async fn connect(&self) -> Result<()> {
        ...
        // ⚠️ 本地补丁（Gosslan 2026-09-13）：把扫描到的**地址类型**带进 `BLEDevice::new`。
        // 上游这里丢掉了 `self.shared.address_type`（扫描回调 `args.BluetoothAddressType()`
        // 明明已经采集了它），而**随机地址的外设必须用带地址类型的 WinRT 重载才连得上**。
        // 详见 `ble/device.rs` 里 `BLEDevice::new` 的文档注释与真机日志。
        // 注意两个 `AddressType` 是不同的类型：这个字段存的是 btleplug 的
        // `api::AddressType`（见 L150 的 `derive_properties`），而 WinRT 那个叫
        // `BluetoothAddressType`，所以要显式映射。
        let address_type = match *self.shared.address_type.read().unwrap() {
            Some(AddressType::Random) => Some(BluetoothAddressType::Random),
            Some(AddressType::Public) => Some(BluetoothAddressType::Public),
            None => None,
        };
