...
// ---------------- 引用解析 ----------------
// 引用消息的 content 首行为 `「引用 {sender}：{snippet}」`，其余为正文。
// 渲染成引用块（左侧竖线 + 灰字），复制/转发仍是完整原文。
const QUOTE_PREFIX = "「引用 ";

const parsed = computed(() => {
  if (!props.content.startsWith(QUOTE_PREFIX)) return { quote: "", body: props.content };
  const nl = props.content.indexOf("\n");
  if (nl < 0 || !props.content.slice(0, nl).trimEnd().endsWith("」")) {
    return { quote: "", body: props.content };
  }
  return {
    quote: props.content.slice(0, nl).trimEnd(),
    body: props.content.slice(nl + 1),
  };
});
</script>
