...
<template>
  <!-- 抖音式表情面板：单分类、上下滚动；正方形格子 + object-contain 保持原图比例。
       ⚠️ 面板宽度与网格是**精确配合**的：
         内宽 = 360 - 16(p-2 左右各 8) = 344 = 8 列 × 36(h-9/w-9) + 7 间隙 × 8(gap-2)
  ...
  <div
    v-if="open"
    class="frost absolute bottom-full left-0 z-50 mb-2 w-[360px] select-none rounded-xl border border-[var(--gosslan-border)] shadow-xl"
    @click.stop
  >
    <div
      class="grid grid-cols-8 content-start gap-2 overflow-y-auto p-2"
      style="height: 300px"
    >
      <button
        v-for="e in EMOJIS"
